...
future_queries_to_use <- function() {
  ...
  storageAmount <- dbGetQuery(con, "select trim(relname) as table_name, col, count(*) as slices
  ...
  blockNumber <- dbGetQuery(con, "SELECT trim(n.nspname), trim(c.relname), c.oid, 
  ...
  rowNumber <- dbGetQuery(con, "select trim(name) as table, stv_blocklist.slice, stv_tbl_perm.rows
